...
def parse_list_page(soup):
    ...
    latin_entries = [table.find('i') for table in tables]
    latin_entries = [
        entries.getText() for entries in latin_entries if entries != None
    ]
    eng_entries = [
        table.find('span', {'class': 'english'}) for table in tables
    ]
    eng_entries = [
        entries.getText() for entries in eng_entries if entries != None
    ]
